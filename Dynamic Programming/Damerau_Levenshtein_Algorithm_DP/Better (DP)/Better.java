...
public class WeightedDamerauLevenshteinDistance {
            ...
            for (int j = 1; j <= n; j++) {
                if (str1.charAt(i - 1) == str2.charAt(j - 1)) {
                    ...
                    if (i > 1 && j > 1 && 
                        ...
                        int trans = dpMatrix[i - 2][j - 2] + costs.transposeCost;
                        dpMatrix[i][j] = minimumCost(ins, del, rep, trans);
                    } else {
    ...
    public static int damerauLevenshteinWeightedCostOnly(String str1, String str2, int insertCost, int deleteCost, int replaceCost, int transposeCost) {
        OperationCosts costs = new OperationCosts(insertCost, deleteCost, replaceCost, transposeCost);
        return damerauLevenshteinWeightedCostOnly(str1, str2, costs);
    }
    ...
    public static DLResult damerauLevenshteinWeightedCostAndOperation(String str1, String str2, OperationCosts costs) {
        ...
        DLResult[][] dpMatrix = new DLResult[m + 1][n + 1];
        
        // Initialize matrix
        for (int i = 0; i <= m; i++) {
            for (int j = 0; j <= n; j++) {
                dpMatrix[i][j] = new DLResult();
            }
        }
        ...
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
                if (str1.charAt(i - 1) == str2.charAt(j - 1)) {
                    ...
                    if (canTranspose) {
                        int trans = dpMatrix[i - 2][j - 2].cost + costs.transposeCost;
                        int minVal = minimumCost(ins, del, rep, trans);
                        dpMatrix[i][j].cost = minVal;
                        
                        if (minVal == trans) {
                            dpMatrix[i][j].operations = new ArrayList<>(dpMatrix[i - 2][j - 2].operations);
                            dpMatrix[i][j].operations.add("TRANSPOSE '" + 
                                str1.charAt(i - 2) + str1.charAt(i - 1) + 
                                "' to '" + str2.charAt(j - 2) + str2.charAt(j - 1) + "'");
                        } else if (minVal == rep) {
                            dpMatrix[i][j].operations = new ArrayList<>(dpMatrix[i - 1][j - 1].operations);
    ...
    // Overloaded version for backward compatibility
    public static DLResult damerauLevenshteinWeightedCostAndOperation(String str1, String str2, int insertCost, int deleteCost, int replaceCost, int transposeCost) {
        OperationCosts costs = new OperationCosts(insertCost, deleteCost, replaceCost, transposeCost);
        return damerauLevenshteinWeightedCostAndOperation(str1, str2, costs);
    }
    ...
    public static int editDistance(String s1, String s2, OperationCosts costs) {
        ...
        if (flag == 1) {
            DLResult result = damerauLevenshteinWeightedCostAndOperation(s1, s2, costs);
        ...
        return damerauLevenshteinWeightedCostOnly(s1, s2, costs);
    }
    
    // Overloaded version for backward compatibility
    public static int editDistance(String s1, String s2, int ci, int cd, int cs, int ct) {
        OperationCosts costs = new OperationCosts(ci, cd, cs, ct);
        return editDistance(s1, s2, costs);
    }
    

    public static void main(String[] args) {
        System.out.println("This is a Java program to calculate the Weighted Damerau-Levenshtein distance."); 
        ...
        for (int i = 1; i <= numInputs; i++) {
            ...
            System.out.println("Damerau-Levenshtein Distance for Pair " + i + ": " + res); 
        }
