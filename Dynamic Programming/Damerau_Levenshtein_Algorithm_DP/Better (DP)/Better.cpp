...
// Time Complexity: O(M * N) - traversing the matrix with transposition checks
        ...
        for (int j = 1; j <= n; j++) {
            if (str1[i - 1] == str2[j - 1]) {
                ...
                if (i > 1 && j > 1 && 
                    ...
                    int trans = dp_Matrix[i - 2][j - 2] + costs.transpose_cost;
                    dp_Matrix[i][j] = minimum_cost(ins, del, rep, trans);
                } else {
...
int damerauLevenshtein_Weighted_Cost_Only(const string& str1, const string& str2, int insert_Cost, int delete_Cost, int replace_Cost, int transpose_Cost) {
    OperationCosts costs(insert_Cost, delete_Cost, replace_Cost, transpose_Cost);
    return damerauLevenshtein_Weighted_Cost_Only(str1, str2, costs);
}
...
DL_Result damerauLevenshtein_Weighted_Cost_And_Operation(const string& str1, const string& str2, const OperationCosts& costs) {
    ...
    vector<vector<DL_Result>> dp_Matrix(m + 1, vector<DL_Result>(n + 1));
    ...
    for (int i = 1; i <= m; i++) {
        for (int j = 1; j <= n; j++) {
            if (str1[i - 1] == str2[j - 1]) {
                ...
                // Check for transposition
                bool canTranspose = (i > 1 && j > 1 && 
                                   str1[i - 1] == str2[j - 2] && str1[i - 2] == str2[j - 1]);
                
                if (canTranspose) {
                    int trans = dp_Matrix[i - 2][j - 2].cost + costs.transpose_cost;
                    int minVal = minimum_cost(ins, del, rep, trans);
                    dp_Matrix[i][j].cost = minVal;

                    if (minVal == trans) {
                        dp_Matrix[i][j].operations = dp_Matrix[i - 2][j - 2].operations;
                        dp_Matrix[i][j].operations.push_back("TRANSPOSE '" + 
                            string(1, str1[i - 2]) + string(1, str1[i - 1]) + 
                            "' to '" + string(1, str2[j - 2]) + string(1, str2[j - 1]) + "'");
                    } else if (minVal == rep) {
...
// Overloaded version for backward compatibility
DL_Result damerauLevenshtein_Weighted_Cost_And_Operation(const string& str1, const string& str2, int insert_Cost, int delete_Cost, int replace_Cost, int transpose_Cost) {
    OperationCosts costs(insert_Cost, delete_Cost, replace_Cost, transpose_Cost);
    return damerauLevenshtein_Weighted_Cost_And_Operation(str1, str2, costs);
}
...
int EditDistance(const string& s1, const string& s2, const OperationCosts& costs) {
    ...
    if (flag == 1) {
        DL_Result result = damerauLevenshtein_Weighted_Cost_And_Operation(s1, s2, costs);
    ...
    return damerauLevenshtein_Weighted_Cost_Only(s1, s2, costs);
}

// Overloaded version for backward compatibility
int EditDistance(string s1, string s2, int Ci, int Cd, int Cs, int Ct) {
    OperationCosts costs(Ci, Cd, Cs, Ct);
    return EditDistance(s1, s2, costs);
}

int main() {
    cout << "This is a C++ program to calculate the Weighted Damerau-Levenshtein distance."; 
    ...
    for (int i = 1; i <= numInputs; i++) {
        ...
        cout << "Damerau-Levenshtein Distance for Pair " << i << ": " << res << endl;  // NEW: Updated output
    }
