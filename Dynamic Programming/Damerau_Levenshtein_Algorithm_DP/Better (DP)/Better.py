...
class OperationCosts:
    def __init__(self, insert_cost: int = 1, delete_cost: int = 1, replace_cost: int = 1, transpose_cost: int = 1): 
...
def minimum_cost_four(a: int, b: int, c: int, d: int) -> int: 
    return min(a, b, c, d)
        ...
        for j in range(1, n + 1):
            if str1[i - 1] == str2[j - 1]:
                ...
                if (i > 1 and j > 1 and 
                    ...
                    transpose = dp_matrix[i - 2][j - 2] + costs.transpose_cost
                    dp_matrix[i][j] = minimum_cost_four(ins, delete, replace, transpose)
                else:
...
def damerau_levenshtein_weighted_cost_and_operation(str1: str, str2: str, costs: OperationCosts) -> DLResult:
    ...
    # Initialize DP matrix with DLResult objects
    dp_matrix = [[DLResult() for _ in range(n + 1)] for _ in range(m + 1)]
    ...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            if str1[i - 1] == str2[j - 1]:
                ...
                # Check for transposition
                can_transpose = (i > 1 and j > 1 and 
                               str1[i - 1] == str2[j - 2] and str1[i - 2] == str2[j - 1])
                
                if can_transpose:
                    transpose = dp_matrix[i - 2][j - 2].cost + costs.transpose_cost
                    min_val = minimum_cost_four(ins, delete, replace, transpose)
                    dp_matrix[i][j].cost = min_val
                    
                    if min_val == transpose:
                        dp_matrix[i][j].operations = dp_matrix[i - 2][j - 2].operations.copy()
                        dp_matrix[i][j].operations.append(
                            f"TRANSPOSE '{str1[i - 2]}{str1[i - 1]}' to '{str2[j - 2]}{str2[j - 1]}'"
                        )
...
def edit_distance(s1: str, s2: str, costs: OperationCosts) -> int:
    ...
    if flag == 1:
        result = damerau_levenshtein_weighted_cost_and_operation(s1, s2, costs)
    ...
    return damerau_levenshtein_weighted_cost_only(s1, s2, costs)


def main():
    print("This is a Python program to calculate the Weighted Damerau-Levenshtein distance.") 
    ...
    for i in range(1, num_inputs + 1):
        ...
        print(f"Damerau-Levenshtein Distance for Pair {i}: {res}") 
