...
def damerau_levenshtein_cost_only(str1, str2, i, j, costs):
    ...
    if str1[i] == str2[j]:
        return damerau_levenshtein_cost_only(str1, str2, i + 1, j + 1, costs)
    ...
    if (i + 1 < len(str1) and j + 1 < len(str2) and 
        str1[i] == str2[j + 1] and str1[i + 1] == str2[j]):
...
def damerau_levenshtein_cost_and_operation(str1, str2, i, j, costs):
    ...
    if i == len(str1):
        ...
        return DLResult(total_cost, ops)
    ...
    if str1[i] == str2[j]:
        result = damerau_levenshtein_cost_and_operation(str1, str2, i + 1, j + 1, costs)
    ...
    if (i + 1 < len(str1) and j + 1 < len(str2) and 
        ...
        transpose_op.operations.insert(0, 
            f"Transpose '{str1[i]}{str1[i + 1]}' to '{str2[j]}{str2[j + 1]}' (cost: {costs.transpose_cost})")
        
        # Find minimum among all four operations
        if (insert_op.cost <= delete_op.cost and insert_op.cost <= replace_op.cost and 
            insert_op.cost <= transpose_op.cost):
            return insert_op
        elif (delete_op.cost <= insert_op.cost and delete_op.cost <= replace_op.cost and 
              delete_op.cost <= transpose_op.cost):
            return delete_op
        elif (replace_op.cost <= insert_op.cost and replace_op.cost <= delete_op.cost and 
              replace_op.cost <= transpose_op.cost):
            return replace_op
        else:
            return transpose_op
...
def edit_distance(s1, s2, costs):
    ...
    if flag == 1:
        result = damerau_levenshtein_cost_and_operation(s1, s2, 0, 0, costs)
    ...
    return damerau_levenshtein_cost_only(s1, s2, 0, 0, costs)

def main():
    print("This is a Python program to calculate the Damerau-Levenshtein distance with custom operation costs.\n\n")
